...
class ACFQLMTAgent(flax.struct.PyTreeNode):
    """Action-chunked FQL with multi-time actor BC flow updates.

    This is a minimal ACFQL baseline for isolating the effect of sampling
    multiple flow times per endpoint action. It keeps the original ACFQL
    critic, distillation, Q loss, best-of-n inference, and chunk handling,
    and only expands the actor BC flow loss to K sampled `(x_t, t)` points
    per data action chunk.
    """
    ...
    def actor_loss(self, batch, grad_params, rng):
        """Compute the ACFQL-MT actor loss."""
        if self.config["action_chunking"]:
...
def get_config():
    ...
    config = ml_collections.ConfigDict(
        dict(
            agent_name='acfql_mt',  # Agent name.
            ...
            n_actor_time_samples=4,  # Number of flow times sampled per action chunk for the BC flow loss.
