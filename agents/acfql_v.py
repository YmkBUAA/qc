...
class ACFQLVAgent(flax.struct.PyTreeNode):
    """Action-chunked FQL with value-advantage weighted BC.

    Q and V take a single start-state s_0 and the flattened action chunk
    a = (a_0, ..., a_{H-1}). The BC flow loss is reweighted by an
    ESS-targeted softmax of A(s_0, a) = Q(s_0, a) - V(s_0).
    There is no noised critic and no Q_n training.
    ...
    #                   E_{a ~ BC}[Q_tgt], which is the BC-weight baseline.
    ...
    # ------------------------------------------------------------------
    # Actor loss — chunked BC flow reweighted by ESS-softmax(Q - V),
    # plus the distill-ddpg head (distill + Q) when configured.
    # ------------------------------------------------------------------
    def actor_loss(self, batch, grad_params, rng):
        ...
        if self.config['action_chunking']:
            per_step_sq = jnp.reshape(
                (pred - vel) ** 2,
                (batch_size, k, self.config['horizon_length'], self.config['action_dim']),
