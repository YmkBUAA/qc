...
class NACFQLAgent(flax.struct.PyTreeNode):
    """Noised Flow Q-learning with Action Chunking (NACFQL).

    Ports NFQL5's dual-boundary Q_n and noised-action-level weighting to
    action-chunked RL. Q, V, and Q_n all take a single start-state s_0 and
    the flattened chunk a = (a_0, ..., a_{H-1}):

        t=0:  Q_n(s_0, x_0, 0) -> V(s_0) = E_{a ~ pi}[Q(s_0, a)]   (V head)
        t=1:  Q_n(s_0, a, 1)   -> Q(s_0, a)                        (anchor)

    The BC flow loss is reweighted by an ESS-targeted softmax of the
    (x_t, t)-conditional advantage a_local = Q(s_0, a) - Q_n(s_0, x_t, t),
    with a trust term on noised-critic disagreement and a dual R^2
    reliability gate (on both Q_n and the main critic).

    Chunking-specific:
      * action_chunking=True  -> a has shape (B, H * action_dim).
      * action_chunking=False -> first action only; H acts as n-step horizon.
      * BC flow loss respects batch['valid'] per chunk step; scalar losses
        use batch['valid'][..., -1] (whole chunk valid).
    """
    ...
    # ------------------------------------------------------------------
    # Critic loss — same chunk-tail TD as ACFQL, plus var_target_q for
    # the critic reliability gate.
    # ------------------------------------------------------------------
    ...
    # ------------------------------------------------------------------
    # ESS-targeted temperature (copied from NFQL5).
    # ------------------------------------------------------------------
    @staticmethod
    ...
    # ------------------------------------------------------------------
    # Actor loss — chunked BC flow reweighted by ESS-softmax(a_local),
    # with trust + dual R^2 gate, plus the distill-ddpg head (distill +
    # Q) so actor_onestep_flow is trained (the V head depends on it).
    # ------------------------------------------------------------------
    ...
    # ------------------------------------------------------------------
    # Noised critic loss — uniform-t anchor to Q(s_0, a) plus V-anchor
    # at t=0 to break the trivial Q_n ≡ Q(s_0, a) fixed point.
    # ------------------------------------------------------------------
    def noised_critic_loss(self, batch, grad_params, rng):
        ...
        # Uniform-t anchor: Q_n(s_0, x_t, t) -> Q(s_0, a) along the flow.
        x_0 = jax.random.normal(x0_rng, (batch_size, n, full_action_dim))
        t = jax.random.uniform(t_rng, (batch_size, n, 1))
        a_expanded = batch_actions[:, None, :]
        ...
        if self.config['encoder'] is not None:
            ...
            noised_qs = noised_qs_flat.reshape(
                (noised_qs_flat.shape[0], batch_size, n)
            )
        else:
            noised_qs = self.noised_network.select('noised_critic')(
        ...
        # V-anchor at t=0: Q_n(s_0, x_0, 0) -> V(s_0).
    ...
    # ------------------------------------------------------------------
    # Combined main-network loss (critic + actor + value). The noised
    # critic is trained on a separate TrainState.
    # ------------------------------------------------------------------
    @jax.jit
    ...
    def create(cls, seed, ex_observations, ex_actions, config):
        ...
        # Noised critic: takes (obs, [x_t, t]) with full_action_dim + 1 features.
...
def get_config():
    config = ml_collections.ConfigDict(
        dict(
            ...
            tau=0.005,
            q_agg='mean',
            # Distill coefficient (chunked BC-to-policy distillation).
            alpha=100.0,
            num_qs=2,
            flow_steps=10,
